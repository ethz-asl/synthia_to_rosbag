...
SynthiaParser::SynthiaParser(const std::string& dataset_path, bool rectified)
    : dataset_path_(dataset_path), rectified_(rectified), initial_pose_set_(false) {
...
bool SynthiaParser::getCameraCalibration(uint64_t cam_id, synthia::CameraCalibration* cam) const {
...
bool SynthiaParser::convertDepthImageToDepthCloud(const sensor_msgs::Image& depth_image_msg,
  ...
  // Set fields
  int offset = 0;
  offset = addPointField(*ptcloud, "x", 1, sensor_msgs::PointField::FLOAT32, offset);
  offset = addPointField(*ptcloud, "y", 1, sensor_msgs::PointField::FLOAT32, offset);
  offset = addPointField(*ptcloud, "z", 1, sensor_msgs::PointField::FLOAT32, offset);
  offset = addPointField(*ptcloud, "rgb", 1, sensor_msgs::PointField::FLOAT32, offset);
  offset = addPointField(*ptcloud, "label", 1, sensor_msgs::PointField::UINT8, offset);
  ...
  for (int v = 0; v < (int)ptcloud->height; ++v, depth_row += row_step) {
    for (int u = 0; u < (int)ptcloud->width; ++u, ++iter_x, ++iter_y, ++iter_z, ++iter_r, ++iter_g,
             ++iter_b, ++iter_label, rgb += color_step, labels += labels_step) {
  ...
  for (pcl::PointCloud<pcl::PointXYZRGB>::iterator it = source_pointcloud->begin();
       it != source_pointcloud->end(); it++) {
    u_int8_t temp = it->b;
    it->b = it->r;
    it->r = temp;
  }
...
bool SynthiaParser::getPoseAtEntry(uint64_t entry, uint64_t* timestamp,
  ...
  std::string filename_cam0 = dataset_path_ + "/" + kPosesFolder + "/Stereo_Left/Omni_F/" +
                              getFilenameForEntry(entry) + ".txt";
  std::string filename_cam1 = dataset_path_ + "/" + kPosesFolder + "/Stereo_Right/Omni_F/" +
                              getFilenameForEntry(entry) + ".txt";
...
bool SynthiaParser::getCameraPoseAtEntry(uint64_t entry, uint64_t id,
                                         synthia::Transformation* pose) {
  // Read in both camera poses and spawn one in between.
  std::string filename_cam = dataset_path_ + "/" + kPosesFolder + "/" + cam_paths_[id] + "/" +
                             getFilenameForEntry(entry) + ".txt";
...
bool SynthiaParser::getImageAtEntry(uint64_t entry, uint64_t cam_id, uint64_t* timestamp,
                                    cv::Mat* image) {
  // Get the timestamp for this first.
  std::string filename =
      dataset_path_ + "/RGB/" + cam_paths_[cam_id] + "/" + getFilenameForEntry(entry) + ".png";
...
bool SynthiaParser::getDepthImageAtEntry(uint64_t entry, uint64_t cam_id, uint64_t* timestamp,
                                         cv::Mat* depth_image) {
  // Get the timestamp for this first.

  std::string filename =
      dataset_path_ + "/Depth/" + cam_paths_[cam_id] + "/" + getFilenameForEntry(entry) + ".png";
...
bool SynthiaParser::getLabelImageAtEntry(uint64_t entry, uint64_t cam_id, uint64_t* timestamp,
                                         cv::Mat* label_image) {
  // Get the timestamp for this first.
  *timestamp = timestamps_ns_[entry];

  std::string filename =
      dataset_path_ + "/GT/COLOR/" + cam_paths_[cam_id] + "/" + getFilenameForEntry(entry) + ".png";
...
bool SynthiaParser::getLabelsAtEntry(uint64_t entry, uint64_t cam_id, uint64_t* timestamp,
                                     cv::Mat* label_image) {
  // Get the timestamp for this first.
...
double SynthiaParser::latToScale(double lat) const { return cos(lat * M_PI / 180.0); }
...
bool SynthiaParser::interpolatePoseAtTimestamp(uint64_t timestamp, synthia::Transformation* pose) {
  ...
  double t = (timestamp - timestamps_ns_[left_index]) /
             static_cast<double>(timestamps_ns_[left_index + 1] - timestamps_ns_[left_index]);
...
size_t SynthiaParser::getNumCameras() const { return camera_calibrations_.size(); }
